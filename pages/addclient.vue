          ...
          <v-toolbar flat>
            ...
            <v-dialog v-model="dialog" max-width="500px">
              <template v-slot:activator="{ on }">
                <v-btn color="grey" class="mb-2 addclient" dense small dark v-on="on">Add Client</v-btn>
              </template>
  ...
  methods: {
    ...
    deleteItem(id) {
      this.$axios.$post(`/api/cdestroy/${id}`).then(() => {
        let i = this.client.map(item => item.id).indexOf(id); // find index of your object
